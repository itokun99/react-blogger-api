...
/** Marks a Blogger comment as spam (`blogger.comments.markAsSpam`); invalidates cached comment lists. */
export function useMarkCommentAsSpam(): UseBloggerMutationResult<MarkCommentAsSpamArgs, Comment> {
  ...
  return useBloggerMutation({
    mutationFn: async (args) => {
      ...
      if (!blogId) {
        throw new Error(
          "useMarkCommentAsSpam requires a blogId - pass args.blogId or set BloggerProvider's defaultBlogId",
        )
      }
      const postId = toPostId(args.postId)
      const commentId = toCommentId(args.commentId)
      const result = await client.comments.markAsSpam({ blogId, postId, commentId })
