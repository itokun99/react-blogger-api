...
import { usePatchPage } from "../../../src/hooks/pages/usePatchPage"
...
describe("usePatchPage", () => {
  test("patches v3/blogs/{blogId}/pages/{pageId} with the given body and resolves with the patched page", async () => {
    ...
    const { result } = renderHook(() => usePatchPage(), { wrapper })

    let patched: Page | undefined
    await act(async () => {
      patched = await result.current.mutateAsync({ pageId: "page-1", page: { title: "Patched" } })
    })

    expect(captured.method).toBe("PATCH")
    expect(captured.url?.pathname).toBe("/v3/blogs/blog-1/pages/page-1")
    expect(captured.body).toEqual({ title: "Patched" })
    expect(patched?.id).toBe("page-1")
  ...
  test("on success, invalidates cached pages:list and pages:get entries for that blogId/pageId", async () => {
    ...
    const { result } = renderHook(() => usePatchPage(), { wrapper: Wrapper })
    ...
    await act(async () => {
      await result.current.mutateAsync({ pageId: "page-1", page: { title: "Patched" } })
    })
