...
import { usePublishPage } from "../../../src/hooks/pages/usePublishPage"
...
describe("usePublishPage", () => {
  test("publishes a page via POST and resolves with the published page", async () => {
    ...
    let published: Page | undefined
    await act(async () => {
      published = await result.current.mutateAsync({ pageId: "page-1" })
    })

    expect(captured.method).toBe("POST")
    expect(captured.url?.pathname).toBe("/v3/blogs/blog-1/pages/page-1/publish")
    expect(published?.id).toBe("page-1")
