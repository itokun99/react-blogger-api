...
import { useMarkCommentAsSpam } from "../../../src/hooks/comments/useMarkCommentAsSpam"
...
describe("useMarkCommentAsSpam", () => {
  test("marks the comment as spam via POST and resolves with the updated comment", async () => {
    ...
    const { result } = renderHook(() => useMarkCommentAsSpam(), { wrapper })
  ...
  test("on success, invalidates cached comments:list and comments:listByBlog entries for the blog", async () => {
    ...
    const { result } = renderHook(() => useMarkCommentAsSpam(), { wrapper: Wrapper })
