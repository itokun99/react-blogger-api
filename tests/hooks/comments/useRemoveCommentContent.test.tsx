...
import { useRemoveCommentContent } from "../../../src/hooks/comments/useRemoveCommentContent"
...
describe("useRemoveCommentContent", () => {
  test("removes the comment content via POST and resolves with the updated comment", async () => {
    const captured: { method: string | null; url: URL | null } = { method: null, url: null }
    const wrapper = providerWrapper(
      async (request) => {
        captured.method = request.method
        captured.url = new URL(request.url)
        return jsonResponse({ kind: "blogger#comment", id: "comment-1", content: undefined })
      },
      { defaultBlogId: "blog-1" },
    )
    const { result } = renderHook(() => useRemoveCommentContent(), { wrapper })
  ...
  test("on success, invalidates cached comments:list and comments:listByBlog entries for the blog", async () => {
    ...
    const { result } = renderHook(() => useRemoveCommentContent(), { wrapper: Wrapper })
    ...
    const unrelatedKey = buildKey(["users", "get", "user-1"])
    ...
    await store.fetchOnce(unrelatedKey, () => Promise.resolve("cached-user"))
    ...
    expect(store.getSnapshot(unrelatedKey).data).toBe("cached-user")
  })
})
