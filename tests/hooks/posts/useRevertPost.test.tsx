...
import { useRevertPost } from "../../../src/hooks/posts/useRevertPost"
...
describe("useRevertPost", () => {
  test("reverts the post with POST and resolves with the reverted post", async () => {
    ...
    const { result } = renderHook(() => useRevertPost(), { wrapper })

    let reverted: Post | undefined
    await act(async () => {
      reverted = await result.current.mutateAsync({ postId: "post-1" })
    })

    expect(captured.method).toBe("POST")
    expect(captured.url?.pathname).toBe("/v3/blogs/blog-1/posts/post-1/revert")
    expect(reverted?.id as string).toBe("post-1")
  ...
  test("on success, invalidates cached posts:list, posts:search and posts:get entries for that blogId/postId", async () => {
    ...
    const { result } = renderHook(() => useRevertPost(), { wrapper: Wrapper })
